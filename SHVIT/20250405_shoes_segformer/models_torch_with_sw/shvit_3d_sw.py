...
from .center_attention_3d import CenterAttention3D
...
class SHCSA3D(nn.Module):
    ...
    def __init__(self, dim: int, qk_dim: int, pdim: int):
        ...
        self.center_att = CenterAttention3D(dim=pdim)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x1 = self.pre_norm(x1)
        x1 = x1.flatten(2).permute(0, 2, 1) # (1,72,28, 28,28) -> (1,72,28*28*28) -> (1, 21952, 72) //////
        # print("x1.shape: ", x1.shape)
        # print("(1,72,28,28,28) -> (1,72,28*28*28) -> (1, 21952, 72) sollte rauskommen!!!!")
        x1 = self.center_att(x=x1, D=D, H=H, W=W)
        # print("============: ", x1.shape)
        x1 = x1.reshape(B, D, H, W, self.pdim).permute(0,4,3,1,2) # reshape(B, C, D, H, W) # -> (1,72,28,28,28) 
    ...
    def __init__(self, dim: int, qk_dim: int, pdim: int, block_type: str, flag_last_stage: bool=False):
        ...
        if block_type == "s":  # for later stages
            ...
            if flag_last_stage == False:
                self.mixer = Residual3D(SHCSA3D(dim=dim, qk_dim=qk_dim, pdim=pdim)) ################################################################################################
            else:            
...
class SHViT3D_SW(nn.Module):
    ...
    def __init__(self, in_channels: int=3, embed_dims=[224, 336, 448], partial_dims=[48, 72, 96],
        ...
        super(SHViT3D_SW, self).__init__()
    ...
    def _build_stage(self):
        ...
        for i, (ed, kd, pd, dpth, do, t) in enumerate(zip(self.embed_dim, self.qk_dim, self.partial_dim, self.depth, self.down_ops, self.types)):
            ...
            for d in range(dpth):
                eval("blocks" + str(i+1)).append(BasicBlock3D(ed, kd, pd, t, flag_last_stage))
            if do[0] == "subsample":
