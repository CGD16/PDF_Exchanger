...
class SHViT(nn.Module):
    ...
    def __init__(self, in_channels: int=3, embed_dims=[224, 336, 448], partial_dims=[48, 72, 96],
        ...
        self.patch_embed = nn.Sequential(
            Conv2d_BN(in_channels=in_channels, out_channels=embed_dims[0]//8, kernel_size=3, strides=2, padding=1), nn.ReLU(),
            Conv2d_BN(in_channels=embed_dims[0]//8, out_channels=embed_dims[0]//4, kernel_size=3, strides=2, padding=1), nn.ReLU(),
            Conv2d_BN(in_channels=embed_dims[0]//4, out_channels=embed_dims[0]//2, kernel_size=3, strides=2, padding=1), nn.ReLU(),
            Conv2d_BN(in_channels=embed_dims[0]//2, out_channels=embed_dims[0]//1, kernel_size=3, strides=2, padding=1), nn.ReLU()
        )
